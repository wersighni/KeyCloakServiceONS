...
class LoginControllerTest {
    ...
    void testLogout_success() throws Exception {
        ...
        when(realmResource.users()).thenReturn(Mockito.mock(UsersResource.class));
        when(realmResource.users().get(userId)).thenReturn(Mockito.mock(UserResource.class));
    ...
    void testLogout_incorrectUserId() throws Exception {
        ...
        when(realmResource.users()).thenReturn(Mockito.mock(UsersResource.class));
        when(realmResource.users().get(userId)).thenThrow(BadRequestException.class);
    ...
    void testCheckVerificationCode_emailNotFound() throws Exception {
        ...
        when(realmResource.users()).thenReturn(Mockito.mock(UsersResource.class));
        when(realmResource.users().searchByEmail(email, true)).thenReturn(new ArrayList<>());
    ...
    void testCheckVerificationCode_noVerificationCodeEmpty() throws Exception {
        ...
        when(realmResource.users().searchByEmail(email, true)).thenReturn(List.of(userRepresentation));
    ...
    void testResetPassword_wrongEmail() throws Exception {
        ...
        try (MockedStatic<SecurityUtils> securityUtilsMockedStatic = Mockito.mockStatic(SecurityUtils.class)) {
            securityUtilsMockedStatic.when(SecurityUtils::getCurrentUserEmail).thenReturn(email);
            when(realmResource.users()).thenReturn(Mockito.mock(UsersResource.class));
            when(realmResource.users()).thenReturn(Mockito.mock(UsersResource.class));
            when(realmResource.users().searchByEmail(email, true)).thenReturn(new ArrayList<>());
    ...
    @Test
    @Transactional
    void testResetPassword_notSameUserEmail() throws Exception {
        final String email = "goodemail@local.host";
        final String password = "goodPassword";

        try (MockedStatic<SecurityUtils> securityUtilsMockedStatic = Mockito.mockStatic(SecurityUtils.class)) {
            securityUtilsMockedStatic.when(SecurityUtils::getCurrentUserEmail).thenReturn("differentemail@local.host");
            when(realmResource.users()).thenReturn(Mockito.mock(UsersResource.class));
            UserRepresentation userRepresentation = new UserRepresentation();
            userRepresentation.setEmail(email);
            when(realmResource.users().searchByEmail(email, true)).thenReturn(List.of(userRepresentation));
            when(realmResource.users().get(userRepresentation.getId())).thenReturn(Mockito.mock(UserResource.class));
            ...
            mockMvc.perform(post("/api/keycloak/auth/findAccount/restPassword")
                    ...
                    .andExpect(status().isBadRequest())
                    .andExpect(jsonPath("$.message")
                            .value("Vous ne pouvez pas réinitialiser le mot de passe d'un autre utilisateur"));
        }
    }
    ...
    void changePassword_notSameUsername() throws Exception {
        ...
        try (MockedStatic<SecurityUtils> securityUtilsMockedStatic = Mockito.mockStatic(SecurityUtils.class)) {
            ...
            when(realmResource.users().searchByEmail(username, true)).thenReturn(List.of(userRepresentation));
            when(realmResource.users().get(userRepresentation.getId())).thenReturn(Mockito.mock(UserResource.class));
    ...
    void changePassword_userNotFound() throws Exception {
        ...
        try (MockedStatic<SecurityUtils> securityUtilsMockedStatic = Mockito.mockStatic(SecurityUtils.class)) {
            ...
            when(realmResource.users()).thenReturn(Mockito.mock(UsersResource.class));
            when(realmResource.users().searchByEmail(username, true)).thenReturn(new ArrayList<>());
    ...
    void changePassword_success() throws Exception {
        ...
        try (MockedStatic<SecurityUtils> securityUtilsMockedStatic = Mockito.mockStatic(SecurityUtils.class)) {
            ...
            UserRepresentation userRepresentation = new UserRepresentation();
            userRepresentation.setEmail(username);
            when(realmResource.users().search(username)).thenReturn(List.of(userRepresentation));
            when(realmResource.users().get(userRepresentation.getId())).thenReturn(Mockito.mock(UserResource.class));
            when(keycloakConfig.instantiateKeycloakUser(loginRequest.getUsername(), loginRequest.getPassword()))
                    .thenReturn(Mockito.mock(Keycloak.class));
            when(keycloakConfig.instantiateKeycloakUser(loginRequest.getUsername(), loginRequest.getPassword())
