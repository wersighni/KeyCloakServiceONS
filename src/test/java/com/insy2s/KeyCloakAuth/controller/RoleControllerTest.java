...
class RoleControllerTest {
    ...
    void testCreate_roleExistInKeycloak_shouldReturn400() throws Exception {
        ...
        when(realmResource.roles()).thenReturn(Mockito.mock(RolesResource.class));
        when(realmResource.roles().get(roleName)).thenReturn(Mockito.mock(RoleResource.class));
        when(realmResource.roles().get(roleName).toRepresentation()).thenReturn(new RoleRepresentation());
    ...
    void testCreate_roleExistInDB_shouldReturn400() throws Exception {
        ...
        when(realmResource.roles()).thenReturn(Mockito.mock(RolesResource.class));
        when(realmResource.roles().get(roleName)).thenReturn(Mockito.mock(RoleResource.class));
        when(realmResource.roles().get(roleName).toRepresentation()).thenReturn(null);
